...
%% handle_cast({update_ets, TableName, KeyName, Value})
%% get an update of an ets record (in the table state) from the main server
...
%% handle_cast({new_simulation_node, PID, Node})
%% message from main server, a new simulation server has been connected
...
%% handle_cast({simulation_node_down,Node})
%% message from main server, a new simulation server has been terminated
...
%% the main server has been terminated
...
insert_ets(Key) ->
  ets:insert(state, {Key, #state{ui_server_pid = [], totalBots=0, liveBots=0, totalWins=0,
